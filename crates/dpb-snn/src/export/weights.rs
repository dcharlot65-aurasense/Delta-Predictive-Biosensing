...
impl LayerWeights {
    ...
    pub fn new(name: String, layer_type: String, weights: Vec<f64>, shape: Vec<usize>) -> Self {
        Self {
            name,
            layer_type,
            weights,
...
impl WeightExporter {
    ...
    fn load_json(data: &[u8]) -> Result<ModelWeights, String> {
        serde_json::from_slice(data).map_err(|e| format!("Failed to deserialize from JSON: {}", e))
    }
