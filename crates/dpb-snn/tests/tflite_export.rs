...
use dpb_snn::export::{
    LayerConfig, LayerType, ModelWeights, QuantizationConfig, TFLiteConfig, TFLiteExporter,
...
fn test_basic_tflite_export() {
    ...
    let layers = vec![LayerConfig::new(
        "fc1".to_string(),
    ...
    assert!(
        !export.model_bytes.is_empty(),
        "Model bytes should not be empty"
    );
...
fn test_quantized_tflite_export() {
    ...
    let config =
        TFLiteConfig::new("test_snn_quantized".to_string()).with_quantization(quant_config);
    ...
    assert!(
        export.metadata.quantized,
        "Model should be marked as quantized"
    );
}
...
fn test_export_without_validation() {
    ...
    let config = TFLiteConfig::new("test_no_validation".to_string()).without_validation();
...
fn test_export_without_metadata() {
    ...
    let config = TFLiteConfig::new("test_no_metadata".to_string()).without_metadata();
