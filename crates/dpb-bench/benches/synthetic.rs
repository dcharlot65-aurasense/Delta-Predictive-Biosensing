...
fn benchmark_ecg_heart_rates(c: &mut Criterion) {
    ...
    for heart_rate in [60.0, 75.0, 100.0, 120.0].iter() {
        let dataset = SyntheticECG::new(size, 250.0, Some(42)).with_heart_rate(*heart_rate);
...
fn benchmark_tremor_frequencies(c: &mut Criterion) {
    ...
    for frequency in [4.0, 5.0, 6.0, 8.0].iter() {
        let dataset = SyntheticTremor::new(size, 100.0, Some(42)).with_frequency(*frequency);
...
fn benchmark_voice_f0(c: &mut Criterion) {
    ...
    for f0 in [100.0, 120.0, 150.0, 200.0].iter() {
        let dataset = SyntheticVoice::new(size, 16000.0, Some(42)).with_f0(*f0);
...
fn benchmark_dataset_throughput(c: &mut Criterion) {
    ...
    let datasets: Vec<(&str, Box<dyn BenchmarkDataset>)> = vec![
        ...
        (
            "tremor",
            Box::new(SyntheticTremor::new(1000, 100.0, Some(42))),
        ),
        (
            "voice",
            Box::new(SyntheticVoice::new(1000, 16000.0, Some(42))),
        ),
    ];

    for (name, dataset) in datasets {
        group.bench_with_input(BenchmarkId::from_parameter(name), &dataset, |b, dataset| {
            b.iter(|| {
                let result = dataset.generate().unwrap();
                black_box(result);
            });
