...
pub enum NeuromorphicTarget {
    ...
    ///
    /// The portable, vendor-neutral target. NIR is actively maintained (v1.0.8,
    /// 2026-07-06) and is consumed by multiple simulators and hardware backends,
    /// so it is the recommended export path — particularly since Intel archived
    /// the Lava framework on 2026-05-13.
    Nir,
...
impl NeuromorphicTarget {
    ...
    pub fn specs(&self) -> HardwareSpecs {
        match self {
            // NIR is an interchange FORMAT, not a device, so it imposes no
            // hardware limits of its own. These values say "unconstrained"
            // rather than inventing plausible-looking silicon numbers — the
            // real constraints belong to whatever backend consumes the file.
            // weight_bits is 64 because NIR stores parameters as float64.
            NeuromorphicTarget::Nir => HardwareSpecs {
                neurons_per_core: u32::MAX,
                cores_per_chip: u32::MAX,
                synapses_per_neuron: u32::MAX,
                weight_bits: 64,
                supports_learning: true,
                supports_dendrites: true,
                time_resolution_us: 0.0, // continuous-time; not quantised by the format
            },
            #[allow(deprecated)]
...
impl NeuromorphicExporter {
    ...
    pub fn export(&self, config: &NetworkConfig) -> Result<String, NeuromorphicExportError> {
        match self.target {
            NeuromorphicTarget::Nir => self.export_nir(config),
            #[allow(deprecated)]
    ...
    /// Export to NIR (Neuromorphic Intermediate Representation).
    ///
    /// Emits a Python script that builds an `nir.NIRGraph` and writes it with
    /// `nir.write()`. Running the script produces a `.nir` HDF5 file consumable
    /// by any NIR-supporting simulator or backend.
    ///
    /// # Unit conversions
    ///
    /// DPB carries biophysical units; NIR expects SI. The emitted script converts
    /// explicitly rather than silently, because a wrong time constant is not
    /// visually obvious in a weight matrix:
    ///
    /// - `tau_m` milliseconds -> seconds (`/ 1000.0`)
    /// - `r` is derived as `tau_m / cm` (from `tau = R*C`), giving megaohms for
    ///   ms/nF inputs
    /// - voltages pass through unchanged, so a network specified in mV stays in
    ///   mV and remains internally consistent
    ///
    /// # Unsupported models
    ///
    /// NIR has no primitive for Izhikevich or AdEx dynamics. Those return
    /// [`NeuromorphicExportError`] rather than emitting a LIF that would silently
    /// misrepresent the network.
    fn export_nir(&self, config: &NetworkConfig) -> Result<String, NeuromorphicExportError> {
        let mut code = String::new();

        writeln!(code, "# DPB network exported to NIR")?;
        ...
        writeln!(code, "import nir")?;
        writeln!(code)?;
        writeln!(code, "nodes = {{}}")?;
        writeln!(code, "edges = []")?;
        writeln!(code)?;

        // Input node sized from the first layer.
        let first = config.layers.first().ok_or_else(|| {
            NeuromorphicExportError::InvalidConfig("network has no layers".to_string())
        })?;
        writeln!(
            code,
            "nodes[\"input\"] = nir.Input(input_type={{\"input\": np.array([{}])}})",
            first.size
        )?;

        // One NIR neuron node per layer.
        for layer in &config.layers {
            let p = &layer.params;
            let tau_s = (p.tau_m / 1000.0) as f64;
            let r = if p.cm.abs() > f32::EPSILON {
                (p.tau_m / p.cm) as f64
            } else {
                1.0
            };
            ...
            match layer.neuron_model {
                NeuronModel::LIF | NeuronModel::ALIF => {
                    writeln!(code, "nodes[\"{}\"] = nir.LIF(", layer.name)?;
                    writeln!(code, "    tau=np.full({n}, {tau_s:.9}),")?;
                    writeln!(code, "    r=np.full({n}, {r:.9}),")?;
                    writeln!(code, "    v_leak=np.full({n}, {:.9}),", p.v_rest)?;
                    writeln!(code, "    v_threshold=np.full({n}, {:.9}),", p.v_thresh)?;
                    writeln!(code, "    v_reset=np.full({n}, {:.9}),", p.v_reset)?;
                    writeln!(code, ")")?;
                }
                NeuronModel::CUBA | NeuronModel::COBA => {
                    let tau_syn_s = (p.tau_syn / 1000.0) as f64;
                    writeln!(code, "nodes[\"{}\"] = nir.CubaLIF(", layer.name)?;
                    writeln!(code, "    tau_syn=np.full({n}, {tau_syn_s:.9}),")?;
                    writeln!(code, "    tau_mem=np.full({n}, {tau_s:.9}),")?;
                    writeln!(code, "    r=np.full({n}, {r:.9}),")?;
                    writeln!(code, "    v_leak=np.full({n}, {:.9}),", p.v_rest)?;
                    writeln!(code, "    v_threshold=np.full({n}, {:.9}),", p.v_thresh)?;
                    writeln!(code, ")")?;
                ...
                ref other => {
                    return Err(NeuromorphicExportError::UnsupportedFeature(format!(
                        "NIR has no primitive for the {other:?} neuron model \
                         (layer '{}'); supported: LIF, ALIF, CUBA, COBA",
                        layer.name
                    )));
                }
            }
        ...
        // Connections become Affine nodes sitting between the layers they join,
        // because NIR models a weight matrix as its own graph node.
        for (i, conn) in config.connections.iter().enumerate() {
            let src = config.layers.iter().find(|l| l.name == conn.source);
            let dst = config.layers.iter().find(|l| l.name == conn.target);
            ...
            writeln!(code, "# {} -> {}", conn.source, conn.target)?;
            match &conn.weights {
                Some(rows) => {
                    writeln!(code, "nodes[\"{w_name}\"] = nir.Affine(")?;
                    write!(code, "    weight=np.array([")?;
                    ...
                    writeln!(code, "], dtype=np.float64),")?;
                    writeln!(code, "    bias=np.zeros({}, dtype=np.float64),", dst.size)?;
                    writeln!(code, ")")?;
                }
                None => {
                    // No explicit matrix: emit a correctly-shaped placeholder and
                    // say so, rather than inventing values that look trained.
                    writeln!(
                        code,
                        "# No weight matrix supplied for this connection \
                         ({:?}); emitting zeros of the correct shape.",
                        conn.conn_type
                    )?;
                    writeln!(code, "nodes[\"{w_name}\"] = nir.Affine(")?;
                    writeln!(
                        code,
                        "    weight=np.zeros(({}, {}), dtype=np.float64),",
                        dst.size, src.size
                    )?;
                    writeln!(code, "    bias=np.zeros({}, dtype=np.float64),", dst.size)?;
                    writeln!(code, ")")?;
            ...
            writeln!(code, "edges.append((\"{}\", \"{w_name}\"))", conn.source)?;
            writeln!(code, "edges.append((\"{w_name}\", \"{}\"))", conn.target)?;
            writeln!(code)?;
        }

        let last = config.layers.last().expect("checked non-empty above");
        writeln!(
            code,
            "nodes[\"output\"] = nir.Output(output_type={{\"output\": np.array([{}])}})",
            last.size
        )?;
        writeln!(code, "edges.append((\"input\", \"{}\"))", first.name)?;
        writeln!(code, "edges.append((\"{}\", \"output\"))", last.name)?;
        writeln!(code)?;
        writeln!(code, "graph = nir.NIRGraph(nodes=nodes, edges=edges)")?;
        writeln!(code, "nir.write(\"{}.nir\", graph)", config.name)?;
        writeln!(code, "print(\"wrote {}.nir\")", config.name)?;
...
#[cfg(test)]
mod nir_export_tests {
    use super::*;

    fn two_layer_net() -> NetworkConfig {
        NetworkConfig {
            name: "dpb_test_net".to_string(),
            layers: vec![
                LayerConfig {
                    name: "l0".into(),
                    size: 4,
    ...
    #[test]
    fn emits_a_runnable_nir_graph() {
        let code = NeuromorphicExporter::new(NeuromorphicTarget::Nir)
            .export(&two_layer_net())
            .expect("NIR export should succeed for an all-LIF network");

        assert!(code.contains("import nir"));
        assert!(code.contains("nir.NIRGraph(nodes=nodes, edges=edges)"));
        assert!(code.contains("nir.write("));
        assert!(code.contains("nir.Input("));
        assert!(code.contains("nir.Output("));
        assert!(code.contains("nir.LIF("));
        assert!(code.contains("nir.Affine("));
    }

    /// tau is specified in milliseconds by DPB and in seconds by NIR. A silent
    /// 1000x error here would not be visible in any weight matrix, so it is
    /// pinned explicitly.
    #[test]
    fn converts_tau_from_milliseconds_to_seconds() {
        let mut cfg = two_layer_net();
        cfg.layers[0].params.tau_m = 20.0; // ms
        let code = NeuromorphicExporter::new(NeuromorphicTarget::Nir)
            .export(&cfg)
            .unwrap();
        assert!(
            code.contains("0.020000000"),
            "20 ms must be emitted as 0.02 s; got:\n{code}"
        );
    }

    /// NIR has no Izhikevich primitive. Emitting a LIF in its place would
    /// silently misrepresent the network, so this must be a hard error.
    #[test]
    fn refuses_models_nir_cannot_represent() {
        let mut cfg = two_layer_net();
        cfg.layers[0].neuron_model = NeuronModel::Izhikevich;
        let err = NeuromorphicExporter::new(NeuromorphicTarget::Nir)
            .export(&cfg)
            .expect_err("Izhikevich has no NIR primitive and must not silently degrade");
        assert!(format!("{err}").contains("Izhikevich"));
    }

    #[test]
    fn nir_is_not_described_as_hardware() {
