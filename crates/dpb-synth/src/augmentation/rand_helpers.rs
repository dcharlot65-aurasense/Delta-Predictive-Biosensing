//! Random number generation helpers for dyn RngCore compatibility
//!
//! These helpers allow generating random values in ranges using only RngCore trait,
...
#[inline]
pub fn random_usize_range(rng: &mut dyn RngCore, min: usize, max: usize) -> usize {
    if min >= max {
        return min;
    }
...
#[inline]
pub fn random_i32_range(rng: &mut dyn RngCore, min: i32, max: i32) -> i32 {
    if min >= max {
        return min;
    }
    let range = (max - min + 1) as u64;
