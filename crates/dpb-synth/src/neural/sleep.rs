...
impl SleepMicrostructureGenerator {
    ...
    pub fn generate_spindle_sequence(&mut self, n_spindles: usize) -> SleepMicroOutput {
        ...
        for i in 0..n_spindles {
            ...
            let spindle_type = if self.rng.gen::<bool>() {
                SpindleType::Fast
            } else {
                SpindleType::Slow
            };
            ...
            SleepMicroPathology::ReducedSpindles { reduction } => {
                ...
                for _ in 0..n_to_remove {
                    if !output.ground_truth.spindles.is_empty() {
                        let idx = self.rng.gen_range(0..output.ground_truth.spindles.len());
    ...
    fn add_spindles(&mut self, signal: &mut [Vec<f64>], duration: f64, density: f64) -> Vec<SpindleInfo> {
        ...
        for _ in 0..n_spindles {
            let onset = self.rng.gen::<f64>() * (duration - 2.0) + 0.5;
            let spindle_type = if self.rng.gen::<bool>() {
                SpindleType::Fast
            } else {
                SpindleType::Slow
    ...
    fn add_single_spindle(
        ...
        let (freq, duration) = match spindle_type {
            SpindleType::Slow => (12.0 + self.rng.gen::<f64>(), 0.5 + self.rng.gen::<f64>() * 0.5),
            SpindleType::Fast => (14.0 + self.rng.gen::<f64>() * 2.0, 0.5 + self.rng.gen::<f64>() * 1.0),
        };

        let amplitude = 30.0 + self.rng.gen::<f64>() * 30.0;
        let n_samples = (duration * self.config.sample_rate) as usize;
    ...
    fn add_single_k_complex(
        ...
        let duration = 0.5 + self.rng.gen::<f64>() * 0.3; // 0.5-0.8s
        let neg_amp = -(100.0 + self.rng.gen::<f64>() * 100.0); // -100 to -200 µV
        let pos_amp = 50.0 + self.rng.gen::<f64>() * 50.0; // 50-100 µV
        ...
        let associated_spindle = self.rng.gen::<f64>() < 0.3;
    ...
    fn add_slow_oscillations(&mut self, signal: &mut [Vec<f64>], duration: f64) -> Vec<SlowOscillationInfo> {
        ...
        let amplitude = 100.0 + self.rng.gen::<f64>() * 50.0;
...
mod tests {
    ...
    fn test_n2_generation() {
        ...
        let mut gen = SleepMicrostructureGenerator::new(config);
        let output = gen.generate_n2(60.0); // 1 minute
    ...
    fn test_n3_generation() {
        ...
        let mut gen = SleepMicrostructureGenerator::new(config);
        let output = gen.generate_n3(60.0);
    ...
    fn test_spindle_sequence() {
        ...
        let mut gen = SleepMicrostructureGenerator::new(config);
        let output = gen.generate_spindle_sequence(5);
    ...
    fn test_kcomplex_sequence() {
        ...
        let mut gen = SleepMicrostructureGenerator::new(config);
        let output = gen.generate_kcomplex_sequence(3);
    ...
    fn test_spindle_types() {
        ...
        let mut gen = SleepMicrostructureGenerator::new(config);
        let output = gen.generate_n2(120.0);
    ...
    fn test_pathological_reduced_spindles() {
        ...
        let normal = gen.generate_n2(60.0);
