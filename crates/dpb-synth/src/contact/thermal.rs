...
impl SyntheticGenerator for SkinTemperatureGenerator {
    ...
    fn generate(
        ...
        let signal = Array1::from_vec(
            (0..n_samples)
                .map(|i| {
                    ...
                    let circadian =
                        params.circadian_amplitude * (circadian_t + params.circadian_phase).sin();
        ...
        gt_params.insert(
            "circadian_amplitude".to_string(),
            params.circadian_amplitude,
        );
        gt_params.insert(
            "vasomotor_amplitude".to_string(),
            params.vasomotor_amplitude,
        );
        gt_params.insert(
            "vasomotor_frequency".to_string(),
            params.vasomotor_frequency,
        );
    ...
    fn validate_params(params: &Self::Parameters) -> crate::Result<()> {
        ...
        if params.baseline_temp < 20.0 || params.baseline_temp > 40.0 {
            return Err(crate::GeneratorError::InvalidParameter(
                "baseline_temp must be 20-40°C".to_string(),
            ));
        }
        Ok(())
    }
...
impl SyntheticGenerator for ThermalResponseGenerator {
    ...
    fn generate(
        ...
        gt_params.insert(
            "num_stimuli".to_string(),
            params.stimulus_times.len() as f64,
        );
    ...
    fn validate_params(params: &Self::Parameters) -> crate::Result<()> {
        ...
        if params.response_latency <= 0.0 {
            return Err(crate::GeneratorError::InvalidParameter(
                "response_latency must be positive".to_string(),
            ));
        }
